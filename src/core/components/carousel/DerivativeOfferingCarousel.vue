...
const setAnimHeight = () => {
  if (inner.value) {
    count.value = Math.ceil(inner.value.clientWidth / 250);
...
<template>
  <div class="relative w-full h-full px-7">
    <div class="absolute w-full h-full top-0 left-0">
      <div class="absolute right-4 h-full flex items-center">
        <font-awesome-icon
          v-if="currentIndex < nfts.length - count"
          :icon="['fas', 'angle-right']"
          class="text-white text-base cursor-pointer hover:text-primary-900 transition"
          @click="next"
        />
      </div>
