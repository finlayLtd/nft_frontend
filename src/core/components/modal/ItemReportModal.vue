...
const report = () => {
  toast.success("This item has been reported.");
  cancel();
};
const cancel = () => {
  $vfm.hide("itemReportModal");
};
...
<template>
  ...
  >
    <div class="bg-tertiary-900 text-white xl:w-home-lg">
      ...
      <div class="text-center relative mt-2">
        <div class="font-press text-xl sm:text-2xl">Report This Item</div>
      </div>
      <div class="text-white mt-7 pb-17.5 px-4 lg:px-17.25">
        <div>
          <div class="font-ibm-bold py-3.5">I think this item is...</div>
          <report-dropdown v-model="category" :list="reasonList" />
        </div>
        <div v-if="category === reasonList[0]">
          <div class="font-ibm-bold py-3.5">Original collection</div>
