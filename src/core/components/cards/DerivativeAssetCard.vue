...
const handleVote = async () => {
  ...
  await nftsService.vote({
    chain: props.asset.contract.chain,
    userId: userId.value,
...
<template>
  <div ref="card" class="w-full parent">
    <div
    ...
    >
      <div class="relative px-4.5 pt-5 pb-8 bg-tertiary-700">
        <div
          class="relative flex items-center gap-4 after:absolute after:top-14 after:h-9.5 after:border-l after:border-l-black after:left-5.5"
        >
          <img
            :src="asset.contract.originNft.contract.imageUrl"
            class="w-11 h-11 bg-tertiary-800"
          />
          <div>
            ...
            >
              {{ asset.contract.originNft.contract.name }}
            </nftmx-trim-string>
          </div>
        </div>
        <div
          class="relative mt-6.5 pl-16 before:absolute before:left-0 before:top-1/2 before:border-t before:border-b before:border-t-black before:border-b-tertiary-600 before:w-8 before:ml-5.5 after:absolute after:top-13.5 after:h-4 after:border-l after:border-l-black after:left-21"
        >
          <div class="flex items-center gap-4">
            <img
              :src="asset.contract.originNft.imageUrl"
              class="w-11.25 h-11.25 bg-tertiary-800"
            />
            <div class="text-11">
              <div class="text-tertiary-400">Locked NFT</div>
              <nftmx-trim-string class="text-white font-ibm-medium">
                {{ asset.contract.originNft.name }}
              </nftmx-trim-string>
            </div>
          </div>
        </div>
	...
	s0.21-0.02,0.31-0.06l5.61-2.34l2.71,1.13L19.53,15.79z"
      ...
      c0.18-0.77-0.2-2.01-0.65-3c-0.03-0.07-0.12-0.07-0.16-0.01l-2.65,4.15L8.72,14.9c-0.04,0.06,0,0.14,0.08,0.14H11.53z"
            ...
            <nftmx-tooltip
              v-if="asset.currentOrder && asset.currentOrder.orderType === '1'"
              :adjust="10"
      ...
      <div class="relative px-4.5 pt-5.5 pb-5 bg-tertiary-700">
        <div class="flex text-white overflow-hidden">
          ...
          <div class="text-xs flex mt-1.5 text-tertiary-400">
            <span class="pr-1">{{ voteCount }}</span>
            <font-awesome-icon
              :icon="['fas', 'thumbs-up']"
              :class="[
                vote ? 'text-primary-900' : '',
        ...
        />
        <div class="text-11 text-white font-ibm mt-4">
        ...
        <div class="flex gap-2 items-center mt-3">
          ...
          />
          <div
            class="text-black font-ibm-medium text-11 whitespace-nowrap text-shadow-h1"
          >
        ...
        <div class="text-11 text-white font-ibm mt-2.5">
        ...
        <div class="text-11 text-white font-ibm mt-2">
          <div class="flex justify-between">
          ...
          <div class="flex justify-between">
            ...
            <div class="flex items-center">
              ...
              <span class="text-primary-900"> Staking </span>
            </div>
          </div>
        </div>
        <div class="h-14 text-tertiary-400 text-11 text-center relative mt-3.5">
          <div
            v-if="asset.currentOrder && asset.currentOrder.orderType === '0'"
          >
            ...
            <div class="flex items-end pt-1.75 w-fit mx-auto max-w-full">
              <div
                class="flex gap-2 items-center text-lg text-primary-900 font-ibm-medium"
              >
                <font-awesome-icon
                  v-if="asset.contract.chain === 'ETHEREUM'"
                  :icon="['fab', 'ethereum']"
    ...
    <router-link
      ...
      :to="{
        ...
        params: {
          chain: asset.contract.chain,
          contractAddress: asset.nfts[selectedAssetIndex].address,
        },
      }"
