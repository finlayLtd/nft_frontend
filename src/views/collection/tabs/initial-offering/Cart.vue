...
const totalPrice = computed(() => {
  ...
  for (let i = 0; i < items.value.length; i++) {
    value += items.value[i].currentOrder.tokenPrice;
  }
  return value;
});
...
const onDrop = (evt, list) => {
  ...
  const index = items.value.findIndex((item) => item.id === movingItem.id);
  if (index < 0) {
    items.value.push(movingItem);
  } else {
    isAdded.value = true;
  }
};
const removeFromCart = (token) => {
  const index = items.value.findIndex((item) => item.id === token.id);
  if (index > -1) {
...
const buyMultiOrders = () => {
  ...
  if (cartChain.value === "ETHEREUM") {
    toast.error("Ethereum network is not ready yet.");
    return;
  }
  const orderIds = items.value.map((item) => item.currentOrder.orderId);
  loading.value = true;
  buyFixedPayOrder(
    items.value[0].currentOrder.contractAddress,
    orderIds,
      ...
      <div class="flex-grow mt-10 relative">
        ...
        >
          ...
          <div class="w-full px-6">
            ...
            >
              <nftmx-img :src="item.imageUrl" class="w-8.75 h-8.75" />
              <div class="flex-grow">
                <div class="text-sm">
