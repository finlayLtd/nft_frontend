...
const followers = ref({ count: 0, users: [], cursor: null });
  ...
  usersService
    ...
    .then((res) => {
      followers.value = {
        ...
        users: [...followers.value.users, ...res.data.users],
...
const initFollowers = () => {
  followers.value = { count: 0, users: [], cursor: null };
  retrieveFollowers();
};
const handleScroll = (e) => {
  if (loading.value || !props.followers?.cursor) {
    return;
  }
...
const follow = (user) => {
  ...
  usersService
    ...
    .then((res) => {
      const index = followers.value.users.findIndex(
        (item) => item.id === user.id
      );
      followers.value.users[index].followers = [userInfo.value];
    })
    .finally(() => {});
};
const unfollow = (user) => {
  ...
  usersService
    ...
    .then((res) => {
      ...
      followers.value.users[index].followers = [];
    })
    .finally(() => {});
};
onMounted(() => {
...
<template>
  ...
  >
    <div class="relative bg-tertiary-900 text-white overflow-auto w-90vw">
      ...
      <div class="text-center font-press text-2xl">Follower List</div>
      <div
        ref="scrollComponent"
        @scroll="handleScroll"
      ...
      >
        <div
          v-for="(user, index) in followers?.users"
          :key="index"
          :class="[
        ...
        <div v-if="followers?.cursor" class="text-center mt-4">Loading...</div>
      </div>
    </div>
  </vue-final-modal>
</template>
