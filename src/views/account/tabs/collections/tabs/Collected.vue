...
import CreateDerivativeModal from "../components/CreateDerivativeModal.vue";
...
const openDerivativeModal = ref(false);
const derivativeAsset = ref();
...
const openDerivative = (token) => {
  derivativeAsset.value = token;
  openDerivativeModal.value = true;
};
...
<template>
  <div class="mb-22">
      ...
      <div class="flex-1">
        ...
        >
          <collected-asset-card
            ...
            @open-derivative="openDerivative"
          />
        </div>
        <div
          v-if="loading"
    ...
    <create-derivative-modal
      v-if="_account && _account.walletAddress === walletAddress"
      v-model="openDerivativeModal"
      :asset="derivativeAsset"
      @derivative-created="removeAsset(derivativeAsset)"
    />
  </div>
</template>
