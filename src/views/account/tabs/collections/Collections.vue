...
onMounted(() => {
  if (accountAddress.value) {
    socket.on(`update-token-stats/${accountAddress.value}`, (res) => {
...
watch(accountAddress, (val, oldVal) => {
  if (oldVal) {
    socket.removeListener(`update-token-stats/${oldVal}`);
  }
  if (val) {
    socket.on(`update-token-stats/${val}`, (res) => {
      itemCounts.value = res;
    });
  }
});
...
<template>
  <div class="relative">
    ...
    <bundled v-if="tab === 'Bundles'" :account="account" />
    <listing
      v-if="tab === 'Listing'"
