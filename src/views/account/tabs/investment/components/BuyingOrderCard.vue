...
const tokenImages = computed(() => {
  if (
    props.order.nft &&
    props.order.nft.nfts &&
    props.order.nft.nfts.length > 0
  ) {
    return props.order.nft.nfts.map((nft) => nft.imageUrl);
  } else {
    return null;
  }
});
    ...
    <div class="w-full relative bg-tertiary-700">
      <div class="hover:shadow-primary-900 transition hover:shadow-default">
        <div class="bg-tertiary-400 relative">
          ...
          >
            <div
              class="absolute top-0 left-0 bg-black w-full h-9.5 flex gap-2 items-center justify-center font-ibm-medium text-sm text-primary-900"
            >
          ...
          <div>
              ...
              <div class="flex justify-between items-end">
                ...
                <div class="flex items-center">
                  <span class="text-primary-900">
                    +{{
                      roundTo(
                        (order.nft.athPrice / (order.nft.baselinePrice || 1)) *
                          100
                      )
                    }}%
                  </span>
              ...
              <div class="flex justify-between items-end">
                ...
                <div class="flex items-center">
                  <span class="text-primary-900 flex items-center gap-1">
                    <font-awesome-icon
                      v-if="order.nft.contract.chain === 'ETHEREUM'"
                      :icon="['fab', 'ethereum']"
                    />
                    <div
                      v-if="order.nft.contract.chain === 'BSC-TESTNET'"
                    ...
                    {{ order ? order.tokenPrice : 0 }}
                  </span>
                  <span class="ml-1 text-tertiary-400">
                    <span>[$</span>
                    <span>
                      {{ order ? roundTo(order.tokenPrice * coinPrice) : 0 }}
                    </span>
                    <span>]</span>
                  </span>
                </div>
            ...
            <div class="text-11 leading-4.5 text-white font-ibm">
              <div class="flex justify-between">
                ...
                <span
                  :class="[
                    order.nft.mlrContract && order.nft.mlrContract.owners
                      ? 'text-primary-900'
                      : 'text-tertiary-400',
                  ]"
                >
                  {{
                    order.nft.mlrContract && order.nft.mlrContract.owners
                      ? order.nft.mlrContract.owners.length
                      : 0
                  }}
                </span>
              </div>
              <div class="flex justify-between">
                ...
                <div class="flex items-center">
                  ...
                  >
                    {{
                      order.nft.mlrContract && order.nft.mlrContract.owners
                        ? order.nft.mlrContract.owners.reduce(
            ...
            <div v-if="order" class="flex gap-2 items-center mt-4.75 mb-2">
              <div
                class="border-t border-t-black border-b border-b-tertiary-600 w-full h-0"
              />
              <div
            ...
            <div v-if="order" class="text-11 leading-4.5 text-white font-ibm">
              <div class="flex justify-between">
                <span class="font-ibm-medium">
                  Period
                  <nftmx-tooltip
                ...
                <span class="text-primary-900">
                  {{ order ? Math.round(order.protectionTime / 86400) : 0 }}
                  Days
                </span>
              </div>
              <div class="flex justify-between">
                ...
                <div class="flex items-center">
                  <span
                    :class="[
                      order && order.protectionRate < 100
                        ? 'text-primary-900'
                        : 'text-tertiary-400',
                    ]"
                  >
                    Slippage
                    {{ order ? 100 - order.protectionRate : 0 }}%
                  </span>
              ...
              <div class="flex-1">
                <div class="text-sm flex items-center">
                  <basic-checkbox
                    ...
                    @check="selectOrder"
                  />
                  <span class="text-xs relative text-tertiary-500 -ml-2">
                    Cancel purchase
                  </span>
                </div>
              </div>
