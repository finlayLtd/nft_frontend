...
const offers = ref([]);
const allCancelledOffers = ref({ offers: [], cursor: null });
const cancelledOffers = ref([]);
const more = computed(
  () =>
    allOffers.value.cursor ||
    offers.value.length < allOffers.value.offers.length
);
const moreCancelled = computed(
  () =>
    allCancelledOffers.value.cursor ||
    cancelledOffers.value.length < allCancelledOffers.value.offers.length
);
const limit = ref(2);
...
const cancelOffer = (id) => {
  ...
  const index1 = allCancelledOffers.value.offers.findIndex(
    (asset) => asset.id === id
  );
  if (index1 > -1) {
    allCancelledOffers.value.offers.splice(index, 1);
    const lines = Math.ceil((cancelledOffers.value.length - 1) / limit.value);
    cancelledOffers.value = allCancelledOffers.value.offers.slice(
      0,
      limit.value * lines
    );
  }
...
const loadMoreCancelledOffers = async () => {
  loading.value = true;
  if (allCancelledOffers.value.cursor) await retrieveCancelledOffers();
  loading.value = false;
  const sliceLimit = limit.value * 3;
  const nextStartPoint = cancelledOffers.value.length;
  cancelledOffers.value = cancelledOffers.value.concat(
    allCancelledOffers.value.offers.slice(
      nextStartPoint,
      nextStartPoint + sliceLimit
    )
  );
};
const initialCancelledOffers = async () => {
  allCancelledOffers.value = { offers: [], cursor: null };
  cancelledOffers.value = [];
  loading.value = true;
  await retrieveCancelledOffers();
  loading.value = false;
  cancelledOffers.value = cancelledOffers.value.concat(
    allCancelledOffers.value.offers.slice(0, limit.value * 4)
  );
};
const retrieveCancelledOffers = async () => {
  return new Promise((resolve, reject) => {
    usersService
      .myOffers({
        ...filterOption.value,
        orderStatus: "5",
        cursor: allCancelledOffers.value.cursor,
        limit: defaultPagination.limit,
        walletAddress: walletAddress.value,
      })
      .then((res) => {
        allCancelledOffers.value = {
          offers: [...allCancelledOffers.value.offers, ...res.data.offers],
          cursor: res.data.cursor,
        };
        resolve(true);
      })
...
const setLimitValue = (width) => {
  ...
  const canceledLines = Math.ceil(offers.value.length / limit.value);
  cancelledOffers.value = allCancelledOffers.value.offers.slice(
    0,
    newLimit * canceledLines
  );
...
<template>
  <div class="flex flex-col sm:flex-row mb-22 xl:pt-7.25">
    <div class="flex-1">
      ...
      <div class="text-white font-ibm-bold text-lg mt-19">
        <span>Cancelled Offers</span>
      </div>
      <div
        v-if="cancelledOffers.length > 0"
        class="grid gap-5 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 3xl:grid-cols-6 mt-9"
      >
        <offer-card
          v-for="(offer, index) in cancelledOffers"
          :key="index"
          :offer="offer"
          @cancel-offer="cancelOffer"
        />
      ...
      <div
        v-if="!loading && cancelledOffers.length === 0"
        class="h-96 font-ibm-bold text-tertiary-500 text-lg flex items-center justify-center"
      >
        No Offers found
      </div>
