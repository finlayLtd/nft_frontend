...
const emit = defineEmits(["filter-offers"]);
...
const addFilterByCollections = (index, collection) => {
  ...
  option.collection = filteredCollections.value.map((item) => item.address);
  emit("filter-offers", option);
};
...
<template>
  <accordion :open="filterActive" class="overflow-hidden text-white">
    <div
      class="bg-tertiary-800 border-t border-b border-black py-5 px-4 sm:px-10 lg:px-22"
    >
      <div
        class="grid gap-4 my-4.75 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5"
      >
        <div>
